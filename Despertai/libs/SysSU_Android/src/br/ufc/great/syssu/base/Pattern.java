...
public class Pattern extends AbstractFieldCollection<PatternField> implements Parcelable {
	...
	public void writeToParcel(Parcel parcel, int flags) {
		...
		for (int i = 0; i < size(); i++) {
	        ...
	        } else if (pf.getType().equals("?array")) { // Suporta apenas array de Strings e de no maximo lengh = 10
		...
		public Pattern createFromParcel(Parcel parcel) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (type.equals("?boolean")) {
		        	...
		        	tValue = (Tuple)parcel.readParcelable(Pattern.class.getClassLoader());
					p = (Pattern) p.addField(name, tValue);
		        } else if (type.equals("?array")) { // Suporta apenas array de Strings e de no maximo lengh = 10
